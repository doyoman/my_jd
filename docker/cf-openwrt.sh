...
# 使用方法：
#     1.创建文件夹  mkdir /usr/dns
#     2.下载脚本并移动到创建好的文件夹中
#     3.在openwrt上系统--计划任务里添加定时运行，如0 9 * * * bash /usr/dns/cf-openwrt.sh
#     4.手动运行一次  bash /usr/dns/cf-openwrt.sh
#     5.套了cf的cdn的节点改为本机IP地址和端口。如IP:192.168.1.1，端口8443
...
#		curl -s -o /dev/null --data "title=$anycast更新成功！&desp=$(date +'%Y-%m-%d %H:%M:%S') %0D%0A%0D%0A---%0D%0A%0D%0A * 优选IP $anycast 满足 $bandwidth Mbps带宽需求 %0D%0A * 公网IP $publicip%0D%0A * 自治域 AS$asn %0D%0A * 经纬度 $longitude,$latitude %0D%0A * META城市 $city %0D%0A * 实测带宽 $realbandwidth Mbps %0D%0A * 峰值速度 $max kB/s %0D%0A * 数据中心 $colo %0D%0A * 总计用时 $((end_seconds-start_seconds)) 秒"  https://sctapi.ftqq.com/$ServerChanTurbo.send #微信推送最新查找的IP-Server酱·Turbo版
		...
		curl -s -o /dev/null -d "chat_id=$chat_id&text=*$anycast更新成功！* %0D%0A$(date +'%Y\-%m\-%d %H:%M:%S')%0D%0A----------------------------------%0D%0A·优选IP $anycast 满足 $bandwidth Mbps带宽需求 %0D%0A·公网IP $publicip%0D%0A·自治域 AS$asn %0D%0A·经纬度 $longitude,$latitude %0D%0A·META城市 $city %0D%0A·实测带宽 $realbandwidth Mbps %0D%0A·峰值速度 $max kB/s %0D%0A·数据中心 $colo %0D%0A----------------------------------&parse_mode=Markdown" "https://api.telegram.org/bot$Telegrambot/sendMessage"
#Telegram推送最新查找的IP- @notification_me_bot
